...
void loadLevel(Level *level, const char *filename)
{
	...
	loadTexture(&level->LevelTexture, textureFilename, 0);
	...
	for(int i = 0; i < level->Width; i++)
	{
		for(int j = 0; j < level->Height; j++)
		{
			...
			// TODO: Move texture S and T into a separate variable, so there's no need for calculation.
...
void drawLevel(Level *level, bool useVBO)
{
	if(useVBO == true)
	{
		drawTextureWithStaticVBO(&level->LevelTexture, 4 * level->Width * level->Height);
	}
	else
	{
		for(int i = 0; i < level->Width; i++)
...
void setLevelLayout(Level *level, int x, int y, int block)
{
	...
	level->Data[x][y][LAYOUT] = block;

	switch(block)
	{
		case 0:
			level->Data[x][y][PROPERTIES] = DEFAULT;
		break;
	...
	// NOTE: Use stride to modify correct indices.
